/**
 * Copyright Stéphane Leduc
 * Distributed under GPLv3 license.
 * See http://www.gnu.org/licenses/gpl-3.0.en.html for more details.
 * Usage
	$('#textContainer').textWave(
		{
			text:'Enjoy JQuery !', // Text to apply the effect
			ratio: 2, // INTEGER
			repeat: 2, // INTEGER or 'loop'
			framerate: 42 // INTEGER [42 is default]
		}
	);
 */;(function ($) {
	var toolbox = {
			...
			/**
				...
				Ex :			[1][2][3][4][3][2][1] -->
						...
						============ < ... 13 loops later over the mask ... it reaches the end of the String > =============
															 ...
															 [j][Q][u][e][r][y][ ][r][o][c][k][s][ ][!]
			*/
			var ref = this, $ref = $(this), data = $ref.data('textWaveData'),
			...
			data = $ref.data('textWaveData', $.extend(data,{maskIndex:(data.maskIndex + data.way)})).data('textWaveData');

			// Should the animation go on again ?
			if( ! data['stop'] && (data.effect == -1 || ( data.completedNb < data.effect)) ){
				setTimeout(	function(){toolbox.play.apply(ref)}, data.framerate );									
			}else{
	...
	$.fn.textWave = function(Opt){
		...
		if(Opt == 'stop'){
			return this.each(function(){
				$(this).data('textWaveData', $.extend($(this).data('textWaveData'),{stop:true}));
			});
		}

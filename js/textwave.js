/**
 * Copyright 2011-2012, Stephane Leduc
 * Distributed under GPL license.
 * See http://www.gnu.org/copyleft/gpl.html for more details.
 */ (function ($) {
		if(! $.fn.textWave){
			$.fn.textWave = function(Opt){
				var config = {'ratio': 2, 'start':'begin', 'framerate': 42}, effects = {'loop':-1, 'custom':2 };

					if(Opt == 'stop'){
						return this.each(function(){
							$(this).data('JQUOpt', $.extend($(this).data('JQUOpt'),{stop:true}));
						});
					}
					$.extend(config,Opt);
					return this.each(function(el,idx){
						var $this = $(this), lettersMap = (config['text'] || ($this.text() ? $this.text() : 'undefined text')).split("");									
						$this.css('line-height', (+($this.css('font-size').match(/^[0-9.]+/).join("")) * config.ratio ) + 'px' );
						...
						$.fn.textWave.play.apply($this.get(0));
					});
			...
			$.fn.textWave.renders = {
				...
				// other effect functions may be added in the future
			};
			$.fn.textWave.play = function(){
				/**
				...
				*/
				...
				data = $ref.data('JQUOpt', $.extend(data,{maskIndex:(data.maskIndex + data.way)})).data('JQUOpt');

				// Should the animation go on again ?
				if( ! data['stop'] && (data.effect == -1 || ( data.completedNb < data.effect)) ){
						setTimeout(	function(){$.fn.textWave.play.apply(ref)}, data.framerate );									
				}else{
					$ref.html($ref.text()).data('JQUOpt',null);
				}
			};
		}
	})(jQuery);
